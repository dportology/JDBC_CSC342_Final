...
public StringBuffer getSkillSummary() throws SQLException
{
    ...
    try
    {
    	...
    	ps=con.prepareStatement(" select age_range, S.skill_id, B.skill_description, count(*) " +
             ...
             " else \'error\' " +
             " end as age_range " +
             " from Team3.person P join Team3.employee E on (p.person_id = e.employee_id)) Z " +
             " join Team3.employee_skills S on (Z.employee_ID = S.employee_ID) join Team3.skill B on (S.skill_id = B.skill_id) " +
